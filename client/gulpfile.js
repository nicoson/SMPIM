...
var paths = {
	...
	venderFonts: 'bower_components/components-font-awesome/webfonts/*',
	venderFontsDest: destDir + '/libs/webfonts',
...
gulp.task("copy", function(){
	...
	gulp.src(paths.venderFonts)
	.pipe(gulp.dest(paths.venderFontsDest)),
...
gulp.task('browserSync', gulp.series('less', 'js', function() {
	//	set server
    browserSync.init({
        server: destDir,
	...
	gulp.watch(paths.appjs, gulp.parallel('js'));
	if(pkg)	gulp.watch(pkg, gulp.parallel('jspkg'));
    gulp.watch(paths.appviews, function(){
		return gulp.src(paths.appviews)
		.pipe(gulp.dest(paths.appviewsDest))
		.pipe(browserSync.reload({stream:true}));
...
gulp.task('default', gulp.series('clean', 'less', 'js', 'jspkg', 'copy'));
gulp.task('test', gulp.series('clean', 'less', 'js', 'jspkg', 'copy', 'jshint', 'qunit'));